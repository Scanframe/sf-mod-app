...
void LineCallback(asIScriptContext* ctx, const DWORD* timeOut);
...
int RunApplication()
{
	...
	asIScriptFunction* func = engine->GetModule(nullptr)->GetFunctionByDecl("float calc(float, float)");
	if (func == nullptr)
	{
	...
	if (r != asEXECUTION_FINISHED)
	{
		...
		else if (r == asEXECUTION_EXCEPTION)
		{
			...
			asIScriptFunction* func_ = ctx->GetExceptionFunction();
			cout << "func: " << func_->GetDeclaration() << endl;
			cout << "modl: " << func_->GetModuleName() << endl;
			cout << "sect: " << func_->GetScriptSectionName() << endl;
...
/*
	...
	if (f == nullptr)
	{
		cout << "Failed to open the script file 'tutorial.as'." << endl;
		return -1;
	}
...
*/
	string script = sf::Resource::getByteArray(":/as/tutorial").toStdString();
	...
	r = mod->AddScriptSection("script", script.c_str(), script.length());
	if (r < 0)
	{
...
void PrintString_Generic(asIScriptGeneric* gen)
{
	auto* str = (string*) gen->GetArgAddress(0);
	cout << *str;
}
