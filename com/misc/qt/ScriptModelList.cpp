...
ScriptModelList::ScriptModelList(QObject* parent)
...
QVariant ScriptModelList::headerData(int section, Qt::Orientation orientation, int role) const
{
...
Qt::ItemFlags ScriptModelList::flags(const QModelIndex& index) const
{
	if (!index.isValid())
	{
...
int ScriptModelList::rowCount(const QModelIndex& parent) const
{
	if (!_interpreter)
	{
		return 0;
...
QVariant ScriptModelList::data(const QModelIndex& index, int role) const
{
	// Sanity check.
...
int ScriptModelList::columnCount(const QModelIndex& parent) const
{
	int rv = 0;
...
bool ScriptModelList::setData(const QModelIndex& index, const QVariant& value, int role)
{
