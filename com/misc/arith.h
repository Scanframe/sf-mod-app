...
// Forward definitions.
class _MISC_CLASS TScriptObject;
class _MISC_CLASS TScriptCalc;
class _MISC_CLASS TMacroCompiler;
...
class TScriptObject
{
	public:
		 ...
		 */
		typedef TScriptObject* TParameters;
		//
		enum EIdentifier
		{
			idUNKNOWN = 0,
		 ...
		 * @param type_name
		 */
		explicit TScriptObject(const char* type_name)
		:TypeName(type_name)
		{}
		/**
		 * Virtual destructor which can be overloaded to clean up objects.
		 */
		virtual ~TScriptObject() = default;
		/**
		 ...
		 */
		virtual bool GetSetValue(const TInfo* info, TValue* value, TValue::vector_type* params, bool flag_set) = 0;
		/**
		...
		/**
		 * Cast operator to be able to return this instance as a TValue.
		 */
		explicit operator TValue() const
		{
			const TScriptObject* so = this;
			if (so)
			{
				return TValue(TValue::vitCUSTOM, &so, sizeof(&so));
			}
	...
	protected:
		 ...
		 */
		void MakeOwner(TScriptObject* obj)
		{
			obj->Owner = this;
		}
	...
	private:
		 ...
		 */
		TScriptObject* Owner{nullptr};

	//DECLARE_REGISTERBASE(TScriptObject);

	friend class TScriptCalc;
	friend class TMacroCompiler;
};

/**
 *
 */
class TScriptCalc :public TScriptObject
{
	public:
		/**
		 * Constructor.
		 ...
		 */
		bool Calculate(const std::string& script, TValue& result);
		/**
	...
	protected:
		 ...
		 */
		TScriptObject* CastToObject(const TValue& value);
		/**
	...
	private:
		...
		struct TDataCode
		{
			 ...
			 * Default constructor.
			 */
			explicit TDataCode(TScriptObject* object)
			{
				Clear(object);
			}
			/**
			 *
			 */
			void Clear(TScriptObject* object)
			{
				Info = nullptr;
				Object = object;
			}
 ...
 */
_MISC_FUNC double ScriptCalc(const std::string& script, double def);

/**
 * Same as above only the passed variable 'x', 'y', 'z' are available as a constant in the script.
 */
_MISC_FUNC double ScriptCalc(const std::string& script, double def,	double x,	double y = 0.0,	double z = 0.0);
