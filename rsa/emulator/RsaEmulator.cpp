...
#define PID_MAP(n)         (n)
#define PID_CHANNEL_MAP(n) ((n) & ~apCH_MASK)
#define PID_GATE_MAP(n)    ((n) & ~apGATE_MASK)
// Macro's for added parameter enumerates PID numbers.
#define PID_ADDED(n)         (apLAST + (n))
#define PID_CHANNEL_ADDED(n) ((apCH_LAST + (n)) & ~apCH_MASK)
#define PID_GATE_ADDED(n)    ((apGATE_LAST + (n)) & ~apGATE_MASK)
// Macro's for mapping result enumerates PID numbers.
#define RID_MAP(n)         (n)
#define RID_CHANNEL_MAP(n) ((n) & ~arCH_MASK)
#define RID_GATE_MAP(n)    ((n) & ~arGATE_MASK)
// Macro's for added result enumerates PID numbers.
#define RID_ADDED(n)         (arLAST + (n))
#define RID_CHANNEL_ADDED(n) ((arCH_LAST + (n)) & ~arCH_MASK)
#define RID_GATE_ADDED(n)    ((arGATE_LAST + (n)) & ~arGATE_MASK)
...
#define PID_REPRATE         PID_CHANNEL_MAP(apCH_REPRATE)
#define PID_SYNCMODE        PID_CHANNEL_MAP(apCH_SYNCMODE)
#define PID_GATES           PID_CHANNEL_MAP(apCH_GATES)
#define PID_INPUTS          PID_CHANNEL_MAP(apCH_INPUTS)
#define PID_TIMEUNITS       PID_CHANNEL_MAP(apCH_TIMEUNITS)
#define PID_COPY_DELAY      PID_CHANNEL_MAP(apCH_COPYDELAY)
#define PID_COPY_RANGE      PID_CHANNEL_MAP(apCH_COPYRANGE)
#define PID_COPY_ENABLE     PID_CHANNEL_MAP(apCH_COPYENABLE)
#define PID_POPMANUAL       PID_CHANNEL_MAP(apCH_POPMANUAL)
#define PID_BIDIRMODE       PID_CHANNEL_MAP(apCH_BIDIRMODE)
...
#define PID_GATE_AMP        PID_GATE_MAP(apGATE_AMP)
#define PID_GATE_TOF        PID_GATE_MAP(apGATE_TOF)
//
...
//
#define RID_POPINDEX        RID_CHANNEL_MAP(arCH_POPINDEX)
#define RID_COPYDATA        RID_CHANNEL_MAP(arCH_COPYDATA)
#define RID_COPYINDEX       RID_CHANNEL_MAP(arCH_COPYINDEX)
//
...
bool PeakNormal
	(
		...
		unsigned threshold, // the threshold value to determine a peak (value > threshold)
		unsigned count,     // number of samples within the gate
		const unsigned char* values,  // array of all values for each sample within the gate
		unsigned& peakidx,  // sample number of the peak value
		unsigned& peakval   // value of the peak
	);
	 ...
	 , SustainEntry(this, &AcquisitionEmulator::Sustain, SustainBase::spTimer, nullptr)
{
	...
	FSingleShotTimer.disable();
	// Set the sustain local timer.
	SustainEntry.setInterval(500);
...
{
	return round<Value::flt_type>(value.getFloat() / ci.TimeUnits, 1.0);
}

int AcquisitionEmulator::GetParamId
	(
		EDefaultParam param,
		unsigned gate,
		unsigned channel
	) const
{
	...
	int id = MAKE_ID
	(
		((param & apCH_MASK) | (param & apGATE_MASK)) ? channel : NO_CHANNEL,
		(param & apGATE_MASK) ? gate : NO_GATE,
		param & ~(apCH_MASK | apGATE_MASK)
	);
	return id;
}

int AcquisitionEmulator::GetResultId
	(
	...
	) const
{
	...
	int id = MAKE_ID
	(
		((result & arCH_MASK) | (result & arGATE_MASK)) ? channel : NO_CHANNEL,
		(result & arGATE_MASK) ? gate : NO_GATE,
		result & ~(arCH_MASK | apGATE_MASK)
	);
	return id;
}
...
{
	double val = ThresholdValue; //calc_offset(ThresholdValue, 0.0, 100.0, 128, true);
	...
	)
{
		...
		switch (pid)
		{
			case PID_ERROR:
				...
				{ // Allow only when the current error is recoverable.
					...
					if (FErrorValue > 0 && setval->getInteger())
					{
						FErrorValue = 0;
					}
				}
			...
			case PID_CHANNELS:
				if (setval)
				{
					FChannelCount = setval->getInteger();
				}
				if (getval)
				{
					getval->set(FChannelCount);
				}
				if (info)
				{
					info->Id = id;
					...
					info->Minimum.set(1);
					info->Maximum.set(4);
					info->Flags = pfSYSTEM | pfEFFECTPARAM | pfEFFECTRESULT;
				}
				break;
			...
			case PID_ONESHOTDELAY:
				if (setval)
				{
					FSingleShotTimer.set(setval->getInteger());
					FSingleShotTimer.disable();
				}
				if (getval)
				{
					getval->set(FSingleShotTimer.getElapseTime().toDouble());
				}
				if (info)
				{
					info->Id = id;
			...
			case PID_ONESHOTSTATE:
				...
				if (getval)
				{
					getval->set(FSingleShotTimer.isEnabled());
				}
				if (info)
				{
					info->Id = id;
			...
			SF_RTTI_NOTIFY(DO_DEFAULT, "Param ID " << stringf("0x%lX", id) << " does not exists!");
				return false;
		}
	}
	else
	{
		...
		// Switch bwtween gate and non gate parameter ID's.
		if (GETGATE(id) == NO_GATE)
		{
			...
			switch (GETINDEX(id))
			{
				default:
					// Report the id was not found.
				SF_RTTI_NOTIFY(DO_DEFAULT, "Channel Param ID " << stringf("0x%lX", id) << " does not exists!");
					return false;

				case PID_REPRATE:
					if (setval)
					{
						ci.RepRate = setval->getInteger();
						if (ci.RepRate)
						{
							SustainEntry.setInterval(1000 / ci.RepRate);
						}
						else
						{
				...
				case PID_GATES:
					...
					if (info)
					{
						...
						info->Flags |= pfEFFECTPARAM | pfEFFECTRESULT;
						if (FlagFixed)
						{
				...
				case PID_COPY_DELAY:
					...
					if (getval)
					{
						getval->set(ci.CopyDelay * ci.TimeUnits);
					}
					if (info)
					{
						info->Id = id;
				...
				case PID_COPY_RANGE:
					...
					if (getval)
					{
						getval->set(ci.CopyRange * ci.TimeUnits);
					}
					if (info)
					{
						info->Id = id;
				...
				case PID_IF_POSITION:
					...
					if (getval)
					{
						getval->set(ci.IfPos * ci.TimeUnits);
					}
					if (info)
					{
						info->Id = id;
				...
				case PID_TCG_DELAY:
					...
					if (getval)
					{
						getval->set(ci.TcgDelay * ci.TimeUnits);
					}
					if (info)
					{
						info->Id = id;
				...
				case PID_TCG_RANGE:
					...
					if (getval)
					{
						getval->set(ci.TcgRange * ci.TimeUnits);
					}
					if (info)
					{
						info->Id = id;
		...
		else
		{
			...
			switch (GETINDEX(id))
			{
				...
				SF_RTTI_NOTIFY(DO_DEFAULT,
					"Gate " << info->Gate << " Param ID " << stringf("0x%lX", id) << " does not exists!");
					return false;
				...
				case PID_GATE_SLAVETO:
					...
					if (info)
					{
						...
						info->States.add(ParamState("Artificial", Value(-2)));
						info->States.add(ParamState("Independant", Value(-1)));
				...
				case PID_GATE_DELAY:
					...
					if (getval)
					{
						getval->set(gi.Delay * ci.TimeUnits);
					}
					if (info)
					{
						info->Id = id;
				...
				case PID_GATE_RANGE:
					...
					if (getval)
					{
						getval->set(gi.Range * ci.TimeUnits);
					}
					if (info)
					{
						info->Id = id;
	...
	// Check if at leat the id is filled in.
	if (info && info->Id == 0)
	{
		return false;
	}
...
bool AcquisitionEmulator::EnumParamIds(IdList& ids)
{
	...
	for (unsigned channel = 0; channel < FChannelCount; channel++)
...
bool AcquisitionEmulator::HandleResult(int id, ResultInfo* info, BufferInfo* bufinfo)
{
	// Check if info must be filled in.
	if (info)
	{
	...
	if (GETCHANNEL(id) == NO_CHANNEL)
	{
		switch (GETINDEX(id))
		{
			default:
				// Report the id was not found.
			SF_RTTI_NOTIFY(DO_DEFAULT, "Result ID " << stringf("0x%lX", id) << " does not exists!");
				return false;
		}
	}
	else
	{
		...
		if (GETGATE(id) == NO_GATE)
		{
			switch (GETINDEX(id))
			{
				default:
					// Report the id was not found.
				SF_RTTI_NOTIFY(DO_DEFAULT, "Channel Result ID " << stringf("0x%lX", id) << " does not exists!");
					return false;
		...
		else
		{
			...
			switch (GETINDEX(id))
			{
				...
				SF_RTTI_NOTIFY(DO_DEFAULT, "Gate " << info->Gate << " Param ID "
					<< stringf("0x%lX", id) << " does not exists!");
					return false;
...
bool AcquisitionEmulator::EnumResultIds(IdList& ids)
{
	...
	for (unsigned channel = 0; channel < FChannelCount; channel++)
	{
		...
		for (unsigned gate = 0; gate < gates; gate++)
		{
			int mid = FChannelInfo[channel].GateInfo[gate].MethodId;
			switch (mid)
			{
				case MID_PEAK:
...
bool AcquisitionEmulator::Sustain(clock_t clk)
{
