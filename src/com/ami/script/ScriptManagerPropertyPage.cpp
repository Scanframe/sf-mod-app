	...
	, _listModel(new ScriptManagerListModel(manager, this))
{
	...
	auto getIndex = [&](const QAbstractItemView* tv) -> QModelIndex {
	...
	connect(_actionAdd, &QAction::triggered, [&] {
		...
		if (index.isValid() && !index.row() &&
				(QMessageBox::Yes == QMessageBox::question(this, tr("New Entry Location"), tr("Add the new entry at the to top?"), QMessageBox::Yes | QMessageBox::No)))
		{
			index = QModelIndex();
		}
