...
void connectIt(PropertySheetDialog* sheet, const QObject* parent, QWidgetList& rv)// NOLINT(misc-no-recursion)
{
	if (!sheet)
	{
		return;
...
*/
	...
	for (auto widget: parent->findChildren<QWidget*>(QString(), Qt::FindChildOption::FindDirectChildrenOnly))
	{
		...
		// GroupBox
		if (const auto o = qobject_cast<QGroupBox*>(widget))
		{
			connectIt(sheet, o, rv);
			continue;
		}
		if (const auto o = qobject_cast<QTabWidget*>(widget))
		{
		...
		{
			QObject::connect(o, &QCheckBox::checkStateChanged, [o, sheet](int) { sheet->checkModified(o); });
			continue;
		}
		// LineEdit
		if (auto o = qobject_cast<QLineEdit*>(widget))
		{
			QObject::connect(o, &QLineEdit::textChanged, [o, sheet](const QString&) { sheet->checkModified(o); });
			continue;
		}
		// TextEdit
		if (auto o = qobject_cast<QTextEdit*>(widget))
		{
			QObject::connect(o, &QTextEdit::textChanged, [o, sheet] { sheet->checkModified(o); });
			continue;
		}
		// QPlainTextEdit
		if (auto o = qobject_cast<QPlainTextEdit*>(widget))
		{
			QObject::connect(o, &QPlainTextEdit::textChanged, [o, sheet] { sheet->checkModified(o); });
			continue;
		}
		// QComboBox
		if (auto o = qobject_cast<QComboBox*>(widget))
		{
			if (o->isEditable())
			{
				QObject::connect(o, &QComboBox::currentTextChanged, [o, sheet](const QString&) { sheet->checkModified(o); });
			}
			else
			{
				QObject::connect(o, &QComboBox::currentIndexChanged, [o, sheet](int) { sheet->checkModified(o); });
			}
			continue;
		}
		// DateTimeEdit
		...
		{
			QObject::connect(o, &QDateTimeEdit::dateTimeChanged, [o, sheet] { sheet->checkModified(o); });
			continue;
		}
		// SpinBox
		if (auto o = qobject_cast<QSpinBox*>(widget))
		{
			QObject::connect(o, &QSpinBox::valueChanged, [o, sheet] { sheet->checkModified(o); });
			continue;
		}
		// DoubleSpinBox
		if (auto o = qobject_cast<QDoubleSpinBox*>(widget))
		{
			QObject::connect(o, &QDoubleSpinBox::valueChanged, [o, sheet] { sheet->checkModified(o); });
			continue;
		}
		// Dial
		if (auto o = qobject_cast<QDial*>(widget))
		{
			QObject::connect(o, &QDial::valueChanged, [o, sheet] { sheet->checkModified(o); });
			continue;
		}
		// KeySequenceEdit
		if (auto o = qobject_cast<QKeySequenceEdit*>(widget))
		{
			QObject::connect(o, &QKeySequenceEdit::keySequenceChanged, [o, sheet] { sheet->checkModified(o); });
			continue;
		}
		// QAbstractItemModel
		if (const auto o = qobject_cast<QTreeView*>(widget))
		{
			// When no model is connected avoid erroneous connections.
			if (o->model())
			{
				QObject::connect(o->model(), &QAbstractItemModel::dataChanged, [sheet, widget] { sheet->checkModified(widget); });
				QObject::connect(o->model(), &QAbstractItemModel::rowsInserted, [sheet, widget] { sheet->checkModified(widget); });
				QObject::connect(o->model(), &QAbstractItemModel::rowsMoved, [sheet, widget] { sheet->checkModified(widget); });
				QObject::connect(o->model(), &QAbstractItemModel::rowsRemoved, [sheet, widget] { sheet->checkModified(widget); });
			}
			continue;
		}
