...
struct PropertySheetDialog::Private : QObject
{
			...
			, _settings(settings)
		{
			...
			struct
			{
					...
					QAction* action;
			} modes[] = {{QListView::IconMode, new QAction(tr("Icons"), menu)}, {QListView::ListMode, new QAction(tr("List"), menu)}};
			...
			connect(ui->listWidget, &QWidget::customContextMenuRequested, [&, menu](QPoint pos) { menu->popup(ui->listWidget->viewport()->mapToGlobal(pos)); });
...
bool PropertySheetDialog::isSheetModified() const
{
	...
	return std::any_of(ppl.begin(), ppl.end(), [](const PropertyPage* p) { return p->isPageModified(); });
}
