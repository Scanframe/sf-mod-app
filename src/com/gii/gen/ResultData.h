 ...
 */
class _GII_CLASS ResultData : public InformationBase
	, public ResultDataTypes
{
	public:
		...
		/**
		 * @brief Reports the status as in the past 'std::ostream'.
		 */
		...
		/**
		 * @brief For owners to writes data to storage in using a range in blocks.
		 *
		 * Written data needs to be committed using #commitValidations() which emits the events to all event handlers.
	...
	public:
		...
		/**
		 * @brief Gets a vector of ranges that are validated.
		 * @return Vector of ranges.
		 */
...
inline ResultData::ResultData(const ResultData& rd)
{
	setup(rd);
}
...
inline bool ResultData::setup(const std::string& definition, id_type id_ofs)
{
	return setup(getDefinition(definition), id_ofs);
}

inline bool ResultData::setup(const ResultData& rd)
{
...
inline ResultData& ResultData::operator=(const ResultData& rd)
{
	if (this != &rd)
	{
		setup(rd);
...
inline void ResultData::setUserData(void* data)
{
	_userData = data;
}

inline void* ResultData::getUserData() const
{
	return _userData;
}

inline ResultData::id_type ResultData::getDesiredId() const
{
	return _desiredId;
}
...
inline bool ResultData::blockRead(const Range& rng, void* dest, bool force) const
{
	return blockRead(rng.getStart(), rng.getSize(), dest, force);
}

inline bool ResultData::blockWrite(const Range& rng, const void* src, bool auto_reserve)
{
	return blockWrite(rng.getStart(), rng.getSize(), src, auto_reserve);
}

inline Range::id_type ResultData::getTransId() const
{
...
inline bool ResultData::requestRange(Range::size_type ofs, Range::size_type sz)
{
...
{
	return _reference->_debug;
}

inline bool ResultData::operator==(const ResultData& rd) const
{
